...
### 2026-06-25 00:23Z
C: dev-studio/github: real Git per agent (clone/branch-with-name/commit/push), PR feature→develop with architect review/merge, release develop→main gated by Approval Engine; GithubService REST client (org PAT from org_integrations); UI phase indicator + read-only code viewer + repo connect + per-agent GitHub card; migration 041 adds repo/identity/PR columns; files=apps/eva-core/src/dev-studio/github/github.service.ts,apps/eva-core/src/dev-studio/claude-code-runner.service.ts,apps/eva-core/src/dev-studio/dev-orchestrator.service.ts,apps/eva-core/src/dev-studio/dev-architect.service.ts,apps/eva-core/src/dev-studio/dev-session.service.ts,apps/eva-core/src/dev-studio/dev-studio.controller.ts,apps/eva-core/src/dev-studio/dev-studio.utils.ts,apps/eva-core/src/dev-studio/dev-studio.types.ts,supabase/migrations/041_dev_studio_github.sql,apps/eva-dashboard/components/dev-studio/code-viewer.tsx,apps/eva-dashboard/components/dev-studio/phase-indicator.tsx,apps/eva-dashboard/components/dev-studio/repo-connect-panel.tsx,apps/eva-dashboard/lib/dev-studio-api.ts; tests=cd apps/eva-core && npm test (769 passed); npm run build; npx tsc --noEmit; cd apps/eva-dashboard && npx eslint components/dev-studio lib/dev-studio-*.ts (clean)
P: pending/improve -> user must apply migration 041 in Supabase; run a live GitHub end-to-end smoke (real PAT + test repo) to confirm agent branches, attributed commits, feature PRs→develop, architect merge, and the develop→main release approval
