...
- Browser: `/browser/open`, `/browser/sessions/:id/{screenshot,click,type,extract-text,extract-table,wait,close,prepare-action,status}`. `SmartNavigatorService` is exported from `BrowserModule` and exposed in the agent tool catalog as `browser_navigate{"session_id","goal","max_steps"?}`, a visual DOM+screenshot observe→decide→act loop with autonomous payment/checkout blocking.
