...
- `supabase/migrations`: SQL migrations currently `001` through `036`.
...
- Agent loop delivery/capability guard: root loop derives explicit user-requested deliverables like generated PDFs and Telegram file sends, raises the step budget to 12 for those tasks, rejects premature `final_answer`, hard-caps web research to 2 successful `web_search` calls for deliverable-heavy tasks, enters a final delivery mode when steps are low, validates outgoing PDFs before Telegram delivery (header/EOF/xref/text/content heuristics), and returns `ok=false` if required delivery is still pending so the runner does not mark the task completed. It also rejects generic “no access to private apps/messages” final answers for authorized WhatsApp/Gmail/Drive/Calendar/Uber/Rappi tools unless a concrete QR/login/approval/setup blocker is named. Multi-phase pipeline retries reuse `task.metadata.pipeline.definition` and completed phase outputs, rerunning only failed/skipped phases.
- WhatsApp deterministic route: explicit screenshot/read/setup/send requests bypass the generic loop and public research; screenshots use the persistent WhatsApp Web profile and publish `task.media`, while actual sends prepare `whatsapp.message.send` approvals. WhatsApp screenshots are immediate with skills/code/self-improvement disabled; sends use the approval horizon. JSON fallback decisions are restricted to the same per-step tool subset as native tool calls.
...
- Request location context: browser/Wear requests may attach fresh, consented coordinates in `task.metadata.request_context.location` (`source`, `latitude`, `longitude`, `accuracy_m`, `captured_at`) with legacy `device_location` compatibility. `AgentRunnerService` uses it for "donde estoy" answers, prompt/tool context, and Uber current-origin resolution; Wear current-location questions force core fallback. Uber quote requests use the deterministic `UberWebService` path before the generic agent loop/web search, extract visible fare candidates from Uber Web, and can fill route form controls when deep links land on the home/search UI. Uber route entry verifies pickup/dropoff field state before Search/See prices using distinctive place tokens, so generic city/CDMX-only dropoff text stays invalid and triggers autocomplete retry; suggestion selection also uses geometry to click visible plain `div` rows near the focused field when roles are missing. Uber ride ordering prepares `uber.ride.order` approval after quote/evidence instead of confirming automatically.
...
- Browser: `/browser/open`, `/browser/sessions/:id/{screenshot,click,type,extract-text,extract-table,wait,close,prepare-action,status}`. `SmartNavigatorService` is exported from `BrowserModule` and exposed in the agent tool catalog as `browser_navigate{"session_id","goal","max_steps"?}`, a Comet-style visual DOM+screenshot observe→decide→act loop with enriched selectors/styles/geometry plus autocomplete/listbox option indexing, progress verification, visual diagnosis after failures/no-progress, task-scoped `tasks.metadata.browser_memory`, compact per-site learnings in `agent_data_log` (`browser:site:<host>`), click/type/scroll/press/back/wait actions, and autonomous payment/checkout blocking.
...
- Dev Studio: `/dev-studio` sessions, goals, iterations, agents, machine lifecycle, events, flow state, and human provisioning endpoints. Claude Code machine OAuth uses `claude auth login` inside the agent container, extracts wrapped/OSC/query-fragment auth URLs, accepts pasted auth codes, and uses either a stored org credential or machine-local auth confirmed by `claude auth status --json`.
- Dev Studio GitHub backing (real repo per session, migration 041): `GithubService` ([dev-studio/github/github.service.ts](file:///Users/djoker/code/eva02/apps/eva-core/src/dev-studio/github/github.service.ts)) is a thin REST client using `fetch` + the org PAT from `org_integrations` (kind=`credential`, provider=`github`, already a known provider) — token never logged, errors token-redacted. Per code task, the orchestrator (`resolveSessionRepoContext`→`setupAgentRepo`) clones into the agent's Docker `/work`, checks out `agent/<name-slug>/iter-N-<slug>` from `origin/develop` with a per-agent git author identity (`agentGitIdentity` in `dev-studio.utils.ts`), runs Claude, then `commitAndPushAgentRepo` and opens a PR feature→develop. The Architect (`DevArchitectService.reviewPullRequest`) approves→squash-merges to develop or requests changes; `createMergeProposal` (now actually called) records the PR (`pr_number/url/head_sha/kind`). Release: on all-goals-complete, `prepareReleasePr` opens `develop → main` (kind=`release`) and requests a level-2 Approval Engine approval (`dev_studio.release_merge`); the orchestrator subscribes to `approval.resolved` → `handleReleaseApprovalResolved` merges to main (`completed`) or closes the PR (`running`). New endpoints: `GET /dev-studio/github/status`, `POST /dev-studio/sessions/:id/repo/connect`, `GET …/repo/tree`, `GET …/repo/file`, `GET /dev-studio/merge-proposals/:id/files`; `merge-proposals/:id/approve|reject` now perform the real GitHub merge/close (release PRs refuse the direct button — Approvals only). Missing token → human task `github_connect`. Dashboard: `phase-indicator`, `repo-connect-panel`, read-only `code-viewer` (tree/file/PR diffs), per-agent GitHub card in `agent-detail-panel`.
...
- Communication/integrations: `communication_channels`, `communication_accounts`, `conversations`, `messages`, `notifications`, `org_integrations`, `mcp_connections`. Telegram outbound file sends compress oversized native videos with `ffmpeg` before applying the 50 MB Bot API limit, and automatically route media types (images, audio, voice) to their corresponding native Bot API endpoints. MCP HTTP/SSE tests detect OAuth-required 401/403 responses and keep tools hidden until auth is connected.
