...
- Request location context: browser/Wear requests may attach fresh, consented coordinates in `task.metadata.request_context.location` (`source`, `latitude`, `longitude`, `accuracy_m`, `captured_at`) with legacy `device_location` compatibility. `AgentRunnerService` uses it for "donde estoy" answers, prompt/tool context, and Uber current-origin resolution; Wear current-location questions force core fallback. Uber quote requests use the deterministic `UberWebService` path before the generic agent loop/web search, extract visible fare candidates from Uber Web, and can fill route form controls when deep links land on the home/search UI. Uber route entry now verifies pickup/dropoff field state before Search/See prices and retries destination autocomplete when placeholders like “Dropoff location” remain visible. Uber ride ordering prepares `uber.ride.order` approval after quote/evidence instead of confirming automatically.
...
- Browser: `/browser/open`, `/browser/sessions/:id/{screenshot,click,type,extract-text,extract-table,wait,close,prepare-action,status}`. `SmartNavigatorService` is exported from `BrowserModule` and exposed in the agent tool catalog as `browser_navigate{"session_id","goal","max_steps"?}`, a Comet-style visual DOM+screenshot observe→decide→act loop with enriched selectors/styles/geometry, progress verification, visual diagnosis after failures/no-progress, task-scoped `tasks.metadata.browser_memory`, compact per-site learnings in `agent_data_log` (`browser:site:<host>`), click/type/scroll/press/back/wait actions, and autonomous payment/checkout blocking.
