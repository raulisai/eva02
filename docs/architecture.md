...
- **Delivery Guard**: For explicit deliverables such as “generate a PDF and send it to Telegram”, the root loop tracks required artifacts/actions, blocks premature `final_answer`, hard-caps repeated web searching, switches to delivery mode when the step budget is low, validates outgoing PDFs before Telegram delivery, and refuses a successful outcome while requested delivery remains pending. Declarative rules in `delivery-requirements.ts` cover PDF, Telegram, email, WhatsApp, Excel, and Calendar events — adding a new deliverable type requires one entry there, not changes in agent-loop.
- **Capability Refusal Guard**: For authorized private-app tools such as WhatsApp, Gmail, Drive, Calendar, Uber, and Rappi, the root loop rejects generic model refusals like “I cannot access your messages” unless the text names a real setup blocker such as QR/login/approval. The model must use the available tool, report the tool observation, or ask for the concrete missing setup step.
- **Deterministic WhatsApp Route**: Explicit WhatsApp screenshots, reads, setup, and message drafts bypass the generic agent loop and public research. Screenshots run immediately through the persistent WhatsApp Web profile and publish `task.media`; real message sends remain approval-gated. The generic loop cannot execute tools removed by adaptive loading through its JSON fallback.
- **Tool Alternatives Map**: `tool-alternatives.ts` provides a static inverse-capability map. When a tool returns an ERROR, the loop appends concrete alternative tools and rationale to the observation so the model pivots on the first failure instead of retrying the same path 2-3 times.
- **Semantic Trajectory Replay**: `agent_trajectories` stores a `goal_embedding vector(1536)` (migration 037). On task completion, the goal is embedded and stored. Subsequent tasks use `match_trajectories` (pgvector RPC) for cosine-similarity retrieval of past successes/failures — replacing the O(N) lexical Jaccard scan that missed paraphrase matches. Falls back to lexical if the column is absent.
- **Medium-Task Deliberation**: For `tier=medium` tasks, `deliberateMediumTask` generates 2-3 candidate approaches with a single `balanced` model call before the first execution step, selects the best one based on available tools and cost, and injects the chosen strategy as an execution brief. This prevents greedy first-step commitment on tasks where the opening decision determines the whole run.
- **Fail-Open Tool Loading**: When no domain signals match in `extractGoalSignals` (zero-signal complex tasks), `selectToolsForPhase` exposes all available tools rather than an empty subset, so a missing keyword never silently limits capability.
...
- **Playwright Browser Runtime**: Handles web automation (like booking Uber rides or checking website states) using persistent browser profiles. `SmartNavigatorService` is exposed to the agent loop as `browser_navigate`, a Comet-style browser agent that sends DOM elements, autocomplete/options, selectors, style/geometry hints, page text, session memory, site learnings, and a screenshot to a vision-capable model on each step. It verifies progress after each action, diagnoses failures/no-progress with a second visual pass, persists task-scoped `browser_memory` in `tasks.metadata`, writes compact per-site learnings to `agent_data_log`, supports click/type/scroll/press/back/wait, and refuses autonomous payment/checkout clicks.
- **WhatsApp Visual Reading**: The runner routes explicit WhatsApp requests directly to `WhatsAppWebService`: screenshot capture publishes the real browser image, DOM extractors handle latest/unread/unanswered/contact reads, and message sends prepare Approval Engine actions. `whatsapp_read` remains available to agent workflows that explicitly require it, with screenshot vision when visual evidence exists.
- **Uber Browser Flow**: Quote-only tasks route through `UberWebService` before the generic agent loop, open Uber Web with the local profile, extract visible fare candidates, and fall back to filling pickup/dropoff form controls when deep links land on the home/search UI. Before clicking Search/See prices, the route form is inspected to verify both pickup and dropoff are filled with distinctive destination tokens, not just generic city/state text; placeholder or city-only states trigger a destination autocomplete retry/first-suggestion selection, including visible plain `div` rows near the focused field when Uber omits accessible roles, or SmartNavigator fallback instead of returning `quote_not_found`. Route preparation can resolve the origin from fresh request location and destination from `known_places` such as `work`; actual ride ordering remains approval-gated through `uber.ride.order`.
