...
- **Delivery Guard**: For explicit deliverables such as “generate a PDF and send it to Telegram”, the root loop tracks required artifacts/actions, blocks premature `final_answer`, switches to delivery mode when the step budget is low, and refuses a successful outcome while requested delivery remains pending.
