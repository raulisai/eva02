...
- [ ] **Live PDF-To-Telegram Smoke**: end-to-end task that researches, generates a PDF in `/work`, sends it through Telegram, confirms `code_execute` + `telegram_send_file` in the task log, and verifies that missing delivery does not transition to `completed`.
