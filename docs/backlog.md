...
- [ ] **Live Location Context Smoke**: from Playground and Wear OS, send "dónde estoy" plus "pídeme un Uber al trabajo"; verify browser/Wear permission prompts, `request_context.location` on the task, no guessed location fallback, known place `work` resolution, and `uber.ride.order` approval before any real booking.
